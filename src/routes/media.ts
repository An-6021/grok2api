...
mediaRoutes.get("/images/:imgPath{.+}", async (c) => {
  ...
  const cached = await c.env.grok2api_cache.getWithMetadata<{ contentType?: string; size?: number }>(key, {
    type: "arrayBuffer",
  });
  if (cached?.value) {
    c.executionCtx.waitUntil(touchCacheRow(c.env.grok2api, key, nowMs()));
  ...
  c.executionCtx.waitUntil(deleteCacheRow(c.env.grok2api, key));

  const settingsBundle = await getSettings(c.env);
  const chosen = await selectBestToken(c.env.grok2api, "grok-4");
    ...
    c.executionCtx.waitUntil(
      (async () => {
        try {
          ...
          await c.env.grok2api_cache.put(key, toKv, {
