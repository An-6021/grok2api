...
async function enforceQuota(args: {
  ...
  const limits = await getApiKeyLimits(args.env.grok2api, key);
...
async function recordImageLog(args: {
  ...
  await addRequestLog(args.env.grok2api, {
    ip: args.ip,
    model: args.model,
...
openAiRoutes.post("/chat/completions", async (c) => {
  ...
  try {
    ...
    for (let attempt = 0; attempt < maxRetry; attempt++) {
      ...
      const customPersonality = !isVideoModel
        ? resolveCustomPersonality({
            body: body as Record<string, unknown>,
            messages: body.messages as any,
            defaultPersonality: settingsBundle.grok.custom_personality_default,
          })
        : null;

      const { content, images } = extractContent(
        body.messages as any,
        isVideoModel ? undefined : { excludeRoles: new Set(["system", "developer"]) },
      );
      ...
      try {
        ...
        if (!upstream.ok) {
          ...
          await recordTokenFailure(c.env.grok2api, jwt, upstream.status, txt.slice(0, 200));
          await applyCooldown(c.env.grok2api, jwt, upstream.status);
        ...
        lastErr = msg;
        await recordTokenFailure(c.env.grok2api, jwt, 500, msg);
        await applyCooldown(c.env.grok2api, jwt, 500);
...
openAiRoutes.post("/images/generations", async (c) => {
  ...
  try {
    ...
    if (stream) {
      ...
      if (!upstream.ok) {
        const txt = await upstream.text().catch(() => "");
        await recordTokenFailure(c.env.grok2api, chosen.token, upstream.status, txt.slice(0, 200));
        await applyCooldown(c.env.grok2api, chosen.token, upstream.status);
        await recordImageLog({
          env: c.env,
          ip,
    ...
    if (imageMethod === IMAGE_METHOD_IMAGINE_WS_EXPERIMENTAL) {
      ...
      if (experimentalToken) {
        ...
        try {
          ...
          const msg = e instanceof Error ? e.message : String(e);
          await recordTokenFailure(c.env.grok2api, experimentalToken.token, 500, msg.slice(0, 200));
          await applyCooldown(c.env.grok2api, experimentalToken.token, 500);
...
openAiRoutes.post("/uploads/image", async (c) => {
  try {
    ...
    await c.env.grok2api_cache.put(kvKey, bytes, {
      expiration: expiresAt,
      metadata: { contentType: mime, size },
    });

    const now = nowMs();
    await upsertCacheRow(c.env.grok2api, {
      key: kvKey,
      type: "image",
      size,
      content_type: mime,
