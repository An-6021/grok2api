...
import { adminFastApiRoutes } from "./routes/admin_fastapi";
import { publicFastApiRoutes } from "./routes/public_fastapi";
...
app.route("/v1/admin", adminFastApiRoutes);
app.route("/v1/public", publicFastApiRoutes);

// Backward-compatible local-cache viewer URLs used by the FastAPI version.
...
// Pages Advanced Mode entry (intentionally not served by Workers assets).
app.get("/_worker.js", (c) => c.notFound());

// UI routes (serve static HTML from app/static)
app.get("/", async (c) => {
  const cfg = await getFastApiConfig(c.env);
  const appCfg = (cfg.app ?? {}) as Record<string, unknown>;
  const enabled = Boolean(appCfg.public_enabled);
  return c.redirect(enabled ? "/login" : "/admin/login", 302);
});

app.get("/admin", (c) => c.redirect("/admin/login", 302));

app.get("/admin/login", (c) => fetchAsset(c, "/admin/pages/login.html"));
...
async function ensurePublicEnabled(c: any): Promise<{ ok: true } | { ok: false; resp: Response }> {
  ...
  const appCfg = (cfg.app ?? {}) as Record<string, unknown>;
  const enabled = Boolean(appCfg.public_enabled);
  if (!enabled) return { ok: false, resp: c.notFound() };
  return { ok: true };
}

app.get("/login", async (c) => {
  const gate = await ensurePublicEnabled(c);
...
app.get("/health", (c) =>
  c.json({
    ...
    bindings: {
      db: Boolean((c.env as any)?.grok2api),
      kv_cache: Boolean((c.env as any)?.grok2api_cache),
