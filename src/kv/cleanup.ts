...
async function deleteKeys(env: Env, keys: string[]): Promise<void> {
  ...
  await Promise.all(keys.map((k) => env.grok2api_cache.delete(k)));
  await deleteCacheRows(env.grok2api, keys);
}
...
export async function runKvDailyClear(env: Env): Promise<{ deleted: number }> {
  ...
  for (let i = 0; i < 200; i++) {
    const rows = await listOldestRows(env.grok2api, null, null, batch);
