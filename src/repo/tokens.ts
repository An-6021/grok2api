...
export async function listTokens(db: Env["grok2api"]): Promise<TokenRow[]> {
...
export async function addTokens(db: Env["grok2api"], tokens: string[], token_type: TokenType): Promise<number> {
  const now = nowMs();
...
export async function deleteTokens(db: Env["grok2api"], tokens: string[], token_type: TokenType): Promise<number> {
  const cleaned = tokens.map((t) => t.trim()).filter(Boolean);
  if (!cleaned.length) return 0;
...
export async function updateTokenTags(db: Env["grok2api"], token: string, token_type: TokenType, tags: string[]): Promise<void> {
...
export async function updateTokenNote(db: Env["grok2api"], token: string, token_type: TokenType, note: string): Promise<void> {
  await dbRun(db, "UPDATE tokens SET note = ? WHERE token = ? AND token_type = ?", [note.trim(), token, token_type]);
}

export async function getAllTags(db: Env["grok2api"]): Promise<string[]> {
...
export async function selectBestToken(db: Env["grok2api"], model: string): Promise<{ token: string; token_type: TokenType } | null> {
  const now = nowMs();
    ...
    const row = await dbFirst<{ token: string }>(
      ...
      `SELECT token FROM tokens
       WHERE token_type = ?
         ...
         AND ${field} != 0
       ORDER BY CASE WHEN ${field} = -1 THEN 0 ELSE 1 END, RANDOM()
       LIMIT 1`,
...
export async function applyCooldown(db: Env["grok2api"], token: string, status: number): Promise<void> {
  const now = nowMs();
