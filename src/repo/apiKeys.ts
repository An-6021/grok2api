...
export async function listApiKeys(db: Env["grok2api"]): Promise<ApiKeyRow[]> {
...
export async function deleteApiKey(db: Env["grok2api"], key: string): Promise<boolean> {
...
export async function batchDeleteApiKeys(db: Env["grok2api"], keys: string[]): Promise<number> {
...
export async function updateApiKeyStatus(db: Env["grok2api"], key: string, is_active: boolean): Promise<boolean> {
  const existing = await dbFirst<{ key: string }>(db, "SELECT key FROM api_keys WHERE key = ?", [key]);
  if (!existing) return false;
...
export async function updateApiKeyName(db: Env["grok2api"], key: string, name: string): Promise<boolean> {
  const existing = await dbFirst<{ key: string }>(db, "SELECT key FROM api_keys WHERE key = ?", [key]);
  if (!existing) return false;
...
export async function validateApiKey(db: Env["grok2api"], key: string): Promise<{ key: string; name: string } | null> {
...
export async function getApiKeyLimits(db: Env["grok2api"], key: string): Promise<ApiKeyLimits | null> {
