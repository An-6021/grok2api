...
export async function upsertCacheRow(db: Env["grok2api"], row: CacheRow): Promise<void> {
  await dbRun(
    db,
...
export async function touchCacheRow(db: Env["grok2api"], key: string, at: number): Promise<void> {
  await dbRun(db, "UPDATE kv_cache SET last_access_at = ? WHERE key = ?", [at, key]);
}

export async function deleteCacheRow(db: Env["grok2api"], key: string): Promise<void> {
  await dbRun(db, "DELETE FROM kv_cache WHERE key = ?", [key]);
}

export async function deleteCacheRows(db: Env["grok2api"], keys: string[]): Promise<void> {
  if (!keys.length) return;
  const placeholders = keys.map(() => "?").join(",");
  await dbRun(db, `DELETE FROM kv_cache WHERE key IN (${placeholders})`, keys);
}

export async function getCacheSizeBytes(db: Env["grok2api"]): Promise<{ image: number; video: number; total: number }> {
